...
class ActividadCompuesta : public Actividad<ActividadCompuesta> {
 private:
  ...
  if (this->hijos.size() > 0) {
    ...
    for (int index = 1; index < hijos.size(); ++index) {
      fechaCandidataInicio =
          this->calcularFecha(fechaCandidataInicio,
                              this->hijos[index]->getFechaPlanteadaInicio(), 1);
      fechaCandidataFinal =
          this->calcularFecha(fechaCandidataFinal,
                              this->hijos[index]->getFechaPlanteadaFinal(), -1);
    }
